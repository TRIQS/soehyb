...
two_band_discrete_bath_helper(double beta, double Lambda, double eps) {
  ...
  nda::array<int, 2> ann_conn = {{2, 0, -1, 1, 3}, {2, 0, -1, 1, 3}, {2, 0, -1, 1, 3}, {2, 0, -1, 1, 3}}; // block column indices of F operators
  nda::array<int, 2> cre_conn = {{1, 3, 0, 4, -1}, {1, 3, 0, 4, -1}, {1, 3, 0, 4, -1}, {1, 3, 0, 4, -1}}; // block column indices of F^dag operators
  ...
  for (int i = 0; i < num_blocks; i++) { h5::read(ad, "sub_hilbert_spaces/" + std::to_string(i) + "/fock_states", subspaces[i]); }
...
nda::array<dcomplex, 4> third_order_02_ppsc(nda::array<dcomplex, 3> Deltat, nda::array<dcomplex, 3> Deltat_refl, imtime_ops &itops,
                                            nda::vector_const_view<double> dlr_it, nda::vector_const_view<double> dlr_rf, double beta, double eps,
                                            nda::array<dcomplex, 3> Gt_dense, nda::array<dcomplex, 3> Fs_dense,
  ...
  fb_zhen      = 0;
  auto third_ffx =
     Sigma_Diagram_calc(Delta_F, Delta_F_reflect, D_02, Deltat, Deltat_refl, Gt_dense, itops, beta, Fs_dense, F_dags_dense, fb_zhen, true);
  auto third_fff =
     Sigma_Diagram_calc(Delta_F, Delta_F_reflect, D_02, Deltat, Deltat_refl, Gt_dense, itops, beta, Fs_dense, F_dags_dense, fb_zhen, false);
...
int main() {
  ...
  for (int fb : fbs) {
    ...
    Sigma_temp = Sigma;
    eval_backbone_fixed_lines_dense(B_02, beta, itops, Deltat, Deltat_refl, Gt_dense, Fs_dense, F_dags_dense, Fset.F_dag_bars, Fset.F_bars_refl,
                                    dlr_it, dlr_rf, T, GKt, Tkaps, Tmu, states, Sigma_L, pole_inds, sign, Sigma);
