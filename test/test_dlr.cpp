...
TEST(strong_coupling, OCA_with_diag_hyb) {
  ...
  for (int i=0;i<r;++i) diagram_3rd_order_true(i,0,1) = OCA_true(i,0,1)*(beta*dlr_it_actual(i)/3.0)*(beta*dlr_it_actual(i)/4.0); 
  diagram_3rd_order_true(_,1,0) = diagram_3rd_order_true(_,0,1);
  diagram_3rd_order_true = diagram_3rd_order_true * dim;//pow(dim,D3.shape(0));
  ...
  auto dlr_it_here = alpha_2*beta*dlr_it_actual;
  auto diagram_4th_order_true = nda::array<dcomplex,3>(Gt.shape());  
  diagram_4th_order_true = 0;
  
  //for (int i=0;i<r;++i) diagram_4th_order_true(i,0,1) = (exp(-(alpha_1+alpha_2)*beta*dlr_it_actual(i)) * (2*dlr_it_actual(i)*(dlr_it_actual(i)-7) + exp(-2*dlr_it_actual(i)) - 4 * exp(-dlr_it_actual(i))*(dlr_it_actual(i)*(dlr_it_actual(i)+4)+8) +31 )/(8*pow(alpha_2,6))); 
  for (int i=0;i<r;++i) diagram_4th_order_true(i,0,1) =diagram_3rd_order_true(i,0,1) *(beta*dlr_it_actual(i)/5.0)*(beta*dlr_it_actual(i)/6.0);
  diagram_4th_order_true(_,1,0) = diagram_4th_order_true(_,0,1);
  diagram_4th_order_true = diagram_4th_order_true * dim;// pow(dim,D4.shape(0));
  // std::cout<<"Error of 4th order diagram is "<<max_element(abs(diagram_4th_order_true - diagram_4th_order)) <<std::endl;
  ...
  std::cout<< "Time spent for calculating diagrams for m = 2,3 is "<<" "<< t2 << " "<<t3<<std::endl;
  ...
  std::cout<<"Time actually spent,    divided by "<< t2<<" is "<<t2/t2<<" "<<int(t3/t2)<<std::endl;
  ...
  std::cout<< "Error of simple OCA_Diagram is "<< max_element(abs(OCAdiagram_simple - OCA_true))<<"; maximum of diagram is "<< max_element(abs(OCA_true)) <<std::endl; 
  begin = std::chrono::high_resolution_clock::now();
  auto diagram_3rd_order_simple = Diagram_calc(Delta_F_simple,D3,Deltat, Gt,itops,beta, F,  F);
  end = std::chrono::high_resolution_clock::now();
  elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - begin);
  double t3_simple =  elapsed.count();
  std::cout<< "Error of simple 3rd orderDiagram is "<< max_element(abs(diagram_3rd_order_simple - diagram_3rd_order_true)) <<"; maximum of diagram is "<< max_element(abs(diagram_3rd_order_true))<<std::endl; 
  begin = std::chrono::high_resolution_clock::now();
  ...
  std::cout<< "Error of simple 4th orderDiagram is "<< max_element(abs(diagram_4th_order_simple - diagram_4th_order_true)) <<"; maximum of diagram is "<< max_element(abs(diagram_4th_order_true))<<std::endl; 
  ...
  diagram_5th_order_true = 0;
  
  for (int i=0;i<r;++i) diagram_5th_order_true(i,0,1) = diagram_4th_order_true(i,0,1)*(beta*dlr_it_actual(i)/7.0) * (beta*dlr_it_actual(i)/8.0);
  diagram_5th_order_true(_,1,0) = diagram_5th_order_true(_,0,1);
  diagram_5th_order_true = diagram_5th_order_true * dim;
  std::cout<< "Error of simple 5th orderDiagram is "<< max_element(abs(diagram_5th_order_simple - diagram_5th_order_true))<<"; maximum of diagram is "<< max_element(abs(diagram_5th_order_true)) <<std::endl; 
  ...
  diagram_6th_order_true = 0;
  
  for (int i=0;i<r;++i) diagram_6th_order_true(i,0,1) = diagram_5th_order_true(i,0,1)*(beta/9)*dlr_it_actual(i) * (beta/10)*dlr_it_actual(i);
  diagram_6th_order_true(_,1,0) = diagram_6th_order_true(_,0,1);
  diagram_6th_order_true = diagram_6th_order_true * dim;//pow(dim,D6.shape(0));
  std::cout<< "Error of simple 6th orderDiagram is "<< max_element(abs(diagram_6th_order_simple - diagram_6th_order_true))<<"; maximum of diagram is "<< max_element(abs(diagram_6th_order_true)) <<std::endl; 
  ...
  diagram_7th_order_true = 0;
  
  for (int i=0;i<r;++i) diagram_7th_order_true(i,0,1) = diagram_6th_order_true(i,0,1)*(beta/11)*dlr_it_actual(i) * (beta/12)*dlr_it_actual(i);
  diagram_7th_order_true(_,1,0) = diagram_7th_order_true(_,0,1);
  diagram_7th_order_true = diagram_7th_order_true * dim;// pow(dim,D7.shape(0));
  std::cout<< "Error of simple 7th orderDiagram is "<< max_element(abs(diagram_7th_order_simple - diagram_7th_order_true)) <<"; maximum of diagram is "<< max_element(abs(diagram_7th_order_true))<<std::endl; 
  ...
  diagram_8th_order_true = 0;
  
  for (int i=0;i<r;++i) diagram_8th_order_true(i,0,1) = diagram_7th_order_true(i,0,1)*(beta*dlr_it_actual(i)/13) * (beta*dlr_it_actual(i)/14);
  diagram_8th_order_true(_,1,0) = diagram_8th_order_true(_,0,1);
  diagram_8th_order_true = diagram_8th_order_true * dim;//pow(dim,D8.shape(0));
  std::cout<< "Error of simple 8th orderDiagram is "<< max_element(abs(diagram_8th_order_simple - diagram_8th_order_true)) <<"; maximum of diagram is "<< max_element(abs(diagram_8th_order_true))<<std::endl; 
  ...
  std::cout<<"m=4      "<< t2*pow(P,4-1)*4/(pow(P,2-1)*2)/1000<<"(prediction)     "<<t4_simple/1000<<std::endl;
  ...
  std::cout<<"m=7      "<< (t2*pow(P,6-1)*7/(pow(P,2-1)*2)/1000)*P<<"(prediction)     "<<t7_simple/1000<<std::endl;
  std::cout<<"m=8      "<< (t2*pow(P,6-1)*8/(pow(P,2-1)*2)/1000)*P*P<<"(prediction)     "<<t8_simple/1000<<std::endl; 
