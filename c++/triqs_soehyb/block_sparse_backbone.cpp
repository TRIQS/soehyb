  ...
  for (int f_ix = 0; f_ix < f_ix_max; f_ix++) {
    ...
    */
    for (int b_ix = 0; b_ix < Gt.get_num_block_cols(); b_ix++) { // loop over blocks of self-energy
      ...
      // block_dims(0) is the same regardless of whether the first vertex has a dagger or not
      ...
      while (w < 2 * m && path_all_nonzero) { // only continue if we have not hit a zero block
        ...
        if (ip == -1 || (w < 2 * m - 1 && Gt.get_zero_block_index(ip) == -1)) { // check if we hit a zero block in F or Gt
          ...
          // block_dims(w + 1) = (backbone.has_vertex_dag(w)) ? Fq.F_dags[0].get_block_size(ip, 0) : Fq.Fs[0].get_block_size(ip, 0);
        }
        w += 1;
      }
      // if (f_ix % static_cast<int>(pow(n * r, m - 1)) == 0) std::cout << "f_ix: " << f_ix << ", b_ix: " << b_ix << ", ind_path: " << ind_path << ", block_dims: " << block_dims << std::endl;
      if (path_all_nonzero) {
